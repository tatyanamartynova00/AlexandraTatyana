...
public class RightSteppingDifferentialOperatorTest {
    ...
    public void testDerive() {
        ...
        assertEquals(differentialOperator.derive(new SqrFunction()).apply(9), (Math.pow((9 + step), 2) - Math.pow(9, 2)) / step);
        assertEquals(differentialOperator.derive(new CubeFunction()).apply(2), (Math.pow((2 + step), 3) - Math.pow(2, 3)) / step);
    }
}
